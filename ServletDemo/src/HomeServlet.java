...
public class HomeServlet extends HttpServlet {
	...
	private String buildLogin(String msg){
		...
		design = "<!DOCTYPE html><html lang='en'><head>"
				...
				+ "<form action='home' method='post'> <input type='hidden' name='branchName' value='Noida'>"
				+"<table> <tr> "
	...
	protected void doPost(HttpServletRequest request
		...
		if(userid.equals(password)){
			...
			response.sendRedirect("welcome.online?bill=10000&userid="+userid+"&branch="+branch);
			//request.setAttribute("bill", 10000);
			//RequestDispatcher rd = request.getRequestDispatcher("welcome");
			//rd.forward(request, response);
//			String urlWithSessionId =
//					response.encodeRedirectURL("welcome");
